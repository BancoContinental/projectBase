...
namespace Continental.API.Seguridad.Repositorio
{
	class TokenRepositorio
	{
		...
		public TokenRepositorio(Settings.Configuraciones configuraciones)
		{
			this.conexionesBD = new ConexionBD(configuraciones.SeteosBD);

			this.connectionStringSeguridad = this.conexionesBD.GetCadenaDeConexion(TiposCredenciales.SERVICIOS_SEGURIDAD, TiposDataSource.DATOSITA);
		}
		...
		public bool ValidateAuthenticationFsys ( string user , string pass )
		{
			...
			try
			{
				string cadena = this.conexionesBD.GetCadenaDeConexion(Crypto.Decrypt(user, true), Crypto.Decrypt(pass, true), TiposDataSource.DATOSITA);
